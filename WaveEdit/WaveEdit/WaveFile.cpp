...
WaveFile * 
WaveFile::getfragment(double start, double end)
{
	double yolo = this->sampleRate / 1000;
	WaveFile * w2 = new WaveFile(numChannels, sampleRate, bitsPerSample);
	for (int a = (start * yolo); a < (end * (this->sampleRate / 1000)); a++) //while
	{
		w2->add_sample(get_sample(a));
	}
...
WaveFile *
WaveFile::removefragment(double start, double end)
{
	double yolo = this->sampleRate / 1000;
	WaveFile * w2 = new WaveFile(numChannels, sampleRate, bitsPerSample);
	for (int a = 0; a < (start * (yolo)); a++) //while
	{
		w2->add_sample(get_sample(a));
	}
	for (int a = (end * (yolo)); a < this->lastSample; a++) //while
	{
		w2->add_sample(get_sample(a));
	}
	w2->updateHeader();
...
WaveFile * WaveFile::insertFragment(WaveFile * n,double start)
{
	WaveFile * w2 = new WaveFile(numChannels, sampleRate, bitsPerSample);
	double yolo = this->sampleRate / 1000;
	int i = 0;
	for (i = 0; i < start*yolo; i++)
	{
		w2->add_sample(get_sample(i));

	}

	for (int j = 0; j < n->lastSample; j++)
	{
		w2->add_sample(n->get_sample(j));
	}
	for (int k = start*yolo; k < lastSample; k++)
	{
		w2->add_sample(get_sample(k));
	}
	w2->updateHeader();
	return w2;
}

WaveFile * WaveFile::replaceFragment(WaveFile * n, double start, double end)
{
	...
	for (int i = 0; i < (start * (yolo)); i++) {
		newWave->add_sample(this->get_sample(i));
	}

	for (int j = 0; j < n->lastSample; j++) {
		newWave->add_sample(n->get_sample(j));
	}

	for (int k = (end * (yolo)); k < this->lastSample; k++) {
		newWave->add_sample(this->get_sample(k));
	}

	return newWave;
}

WaveFile * WaveFile::reverseFragment()
{
	WaveFile * nWave = new WaveFile(numChannels, sampleRate, bitsPerSample);

	for (int i = this->lastSample; i > 0; i--) {
		nWave->add_sample(this->get_sample(i));
	}
